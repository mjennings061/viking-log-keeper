...
def format_data_for_table(raw_df: pd.DataFrame) -> pd.DataFrame:
    ...
    flight_time_sum = grouped.agg(
        FlightTime=("FlightTime", "sum"), PLFs=("PLF", "sum")
    ).reset_index()
    ...
    data_df["PLFs"] = data_df["PLFs"].apply(lambda x: "" if x == 0 else x)
...
def plot_launches_by_commander(df: pd.DataFrame):
    ...
    chart = (
        alt.Chart(launches_by_commander)
        .mark_bar()
        .encode(
...
def plot_firstlast_launch_table(df: pd.DataFrame):
    ...
    first_last_launch = (
        df.groupby("Date")["TakeOffTime"].agg(["min", "max"]).reset_index()
    )
    ...
    first_last_launch = (
        first_last_launch.sort_values(by="Date", ascending=False)
        .reset_index(drop=True)
        .head(n_rows_to_display)
    )
...
def get_aircraft_weekly_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    gur_helper = (
        df.groupby(["Week Start", "Aircraft"])
        .agg(
            {
...
def generate_aircraft_daily_summary(df: pd.DataFrame):
    ...
    # Rename columns
    gur_helper.columns = ["Date", "Aircraft", "Launches", "Flight Time"]
...
def plot_monthly_launches(df: pd.DataFrame):
    ...
    month_df = (
        ...
        .agg(Launches=("Date", "count"), FlightTime=("FlightTime", "sum"))
        .reset_index()
    )
    ...
    melt_df = (
        ...
        .sort_values(by=["YearMonth", "Metric"])
        .reset_index(drop=True)
    )
    ...
    bar_launches = (
        base.transform_filter(alt.datum.Metric == "Launches")
        .mark_bar()
        .encode(
    ...
    bar_flight_time = (
        base.transform_filter(alt.datum.Metric == "FlightTime")
        .mark_bar()
        .encode(
    ...
    combined_chart = alt.layer(bar_launches, bar_flight_time).resolve_scale(
        y="independent"
    )
    ...
    final_chart = alt.hconcat(combined_chart, legend).resolve_legend(
        color="independent"
    )
...
def table_all_launches(df: pd.DataFrame):
    ...
    selected_duties = st.sidebar.multiselect(
        "Filter by Duty", options=unique_duties, default=unique_duties
    )

    # Create a filter for the Aircraft column.
    unique_aircraft = df["Aircraft"].unique()
    selected_aircraft = st.sidebar.multiselect(
        "Filter by Aircraft", options=unique_aircraft, default=unique_aircraft
    )
    ...
    selected_spc = st.sidebar.multiselect(
        "Filter by sortie profile code", options=spc, default=spc
    )
...
def show_logbook_helper(df: pd.DataFrame, commander: str):
    ...
    if commander:
        ...
        sct_df = second_pilot_df[
            second_pilot_df["Duty"].str.contains("SCT|AGT", case=False)
        ]
...
def quarterly_summary(df: pd.DataFrame, commander: str, quarter: str) -> pd.DataFrame:
    ...
    # is second pilot.
    sct_df = df[df["Duty"].str.contains("SCT|AGT", case=False)]
    ...
    sct_in_quarter = sct_df[sct_df["Date"].dt.to_period("Q") <= quarter]
...
def show_launch_delta_metric(df: pd.DataFrame):
    ...
    launches_fy = total_launches_for_financial_year(df=df, year=financial_year)
...
def plot_gif_bar_chart(df: pd.DataFrame):
    ...
    gif_year_df["WeekStart"] = gif_year_df["Date"] - pd.to_timedelta(
        gif_year_df["Date"].dt.weekday, unit="D"
    )

    # Group by week and sum the GIFs flown.
    gif_year_df = (
        gif_year_df.groupby("WeekStart")
        .agg(GIFsFlown=("GIFs Flown", "sum"))
        .reset_index()
    )
    ...
    base = (
        alt.Chart(gif_year_df)
        .mark_area(color="blue")
        .encode(
            x=alt.X(
                "WeekStart:T",
                title="Week Start",
...
def table_gur_summary(aircraft_df: pd.DataFrame, launches_df: pd.DataFrame):
    ...
    last_weekly_summary = gur_summary.drop_duplicates(
        subset="Aircraft", keep="first"
    ).reset_index(drop=True)

    # Merge the first entry with last_entry_df.
    last_entry_df = last_entry_df.merge(
        last_weekly_summary, on="Aircraft", how="left", suffixes=("", "_first")
    )
